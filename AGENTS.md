...
- 提交前必须执行单元测试与集成/E2E测试；如项目未配置集成/E2E脚本，需在提交说明中注明原因与替代验证方式。
...
- 必须在提交前完成集成/E2E测试（若无脚本需说明原因与替代验证）。
- 无无关改动，未引入多余依赖。
- 文档同步更新（如涉及 API/行为变化）。
- 关键功能手动抽检通过（如自动化未覆盖）。
...
- Monorepo roots: desktop (`packages/app` + `packages/app/src-tauri`), mobile (`packages/mobile`), shared contracts (`packages/shared`), reusable readers/tabs (`packages/foliate-js`, `packages/app-tabs`).
- Keep dependency flow downward: `types/contracts` -> `services/data` -> `state/hooks` -> `UI/runtime`.
- Keep native boundaries explicit: Tauri calls through service/lib wrappers.

See `ARCHITECTURE.md` for module map and dependency rules.

### Boundaries

ALWAYS:
- Keep changes minimal and scoped to one package/module unless explicitly required.
- Run `pnpm run check` and `pnpm run verify` before considering work complete.
- Keep `CLAUDE.md`, `AGENTS.md`, `.github/copilot-instructions.md` command blocks synchronized.

ASK:
- Add/update dependencies or lockfile strategy.
- Change shared API contracts used by both app and mobile.
- Change CI/release workflow behavior or Tauri signing/release settings.

NEVER:
- Bypass tests/checks for behavior-changing code.
- Commit secrets, tokens, or local credential files.
- Move Tauri/native concerns directly into UI components.

### Harness Maintenance

1. Module added/moved -> update `ARCHITECTURE.md` module map.
2. Command changed -> update all three instruction files and `package.json` scripts.
3. New architecture decision -> add ADR in `docs/adr/`.
4. Recurring agent mistake -> add rule to ALWAYS/ASK/NEVER.

<!-- EVOLVE: tighten lint gate from typecheck-only to full lint once backlog is resolved -->
