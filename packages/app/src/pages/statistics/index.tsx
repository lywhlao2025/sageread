  ...
  useEffect(() => {
    const loadStats = async () => {
      try {
        ...
        setError(err instanceof Error ? err.message : t("stats.loadFailedDetail", "加载统计数据失败"));
        console.error(t("stats.loadFailedDetail", "加载统计数据失败"), err);
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="container mx-auto p-4">
        <div className="rounded-lg border border-red-200 bg-red-50 p-6 dark:border-red-800 dark:bg-red-950">
          <div className="text-center text-red-700 dark:text-red-300">
            <p className="font-medium text-lg">{t("stats.loadFailed", "加载失败")}</p>
  ...
  return (
    <div className="flex-1 space-y-6 overflow-auto p-3">
      <div className="space-y-2">
        <h1 className="font-bold text-3xl text-neutral-900 dark:text-neutral-100">{t("stats.title", "阅读统计")}</h1>
        <p className="text-neutral-600 dark:text-neutral-400">{t("stats.subtitle", "查看您的阅读习惯和统计数据")}</p>
      </div>

      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <StatCard
          title={t("stats.totalSessions", "总阅读会话")}
          value={totalSessions.toString()}
          icon={<BookIcon className="h-4 w-4" />}
          description={t("stats.totalSessions.desc", "累计阅读次数")}
        />
        <StatCard
          title={t("stats.totalDuration", "总阅读时长")}
          value={`${Math.floor(totalDuration / 3600)}h ${Math.floor((totalDuration % 3600) / 60)}m`}
          icon={<ClockIcon className="h-4 w-4" />}
          description={t("stats.totalDuration.desc", "累计阅读小时数")}
        />
        <StatCard
          title={t("stats.activeDays", "活跃天数")}
          value={totalDays.toString()}
          icon={<CalendarIcon className="h-4 w-4" />}
          description={t("stats.activeDays.desc", "有阅读记录的天数")}
        />
        <StatCard
          title={t("stats.avgSessions", "平均每日会话")}
          value={averageSessionsPerDay.toFixed(1)}
          icon={<TrendingUpIcon className="h-4 w-4" />}
          description={t("stats.avgSessions.desc", "活跃日平均会话数")}
        />
      </div>

      <div className="rounded-lg border border-neutral-150 p-4 dark:border-neutral-800">
        <div className="space-y-2 pb-4">
          <h3 className="font-semibold text-lg text-neutral-900 dark:text-neutral-100">{t("stats.heatmap", "阅读活动热力图")}</h3>
          <p className="text-neutral-600 text-sm dark:text-neutral-400">
            {t("stats.heatmap.desc", "过去一年的阅读活动分布，颜色深浅表示当天的阅读强度")}
          </p>
        </div>
