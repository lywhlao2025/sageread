<div align="center">


## 🚗SageRead X

**官方介绍页**
https://lywhlao2025.top/


**一款支持 AI 对话的电子书阅读器**

[![License](https://img.shields.io/badge/license-AGPL--3.0-blue.svg)](LICENSE) [![Platform](https://img.shields.io/badge/platform-macOS%20%7C%20Windows-lightgrey)](https://github.com/yourusername/sageread) [![PRs Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen.svg)](https://github.com/yourusername/sageread/pulls)

</div>

<br/>
SageRead X 是一款电子书阅读器，主要特点是可以在阅读时随时跟 AI 对话。
你可以用它来读电子书，做笔记和高亮标注。不懂的地方直接问 AI，它会根据书籍内容给你解答。支持自定义 AI 提示词，可以让 AI 帮你总结、翻译或做其他事情。所有数据都存在本地，支持自己配置 AI 服务。

## 👨‍🏫关于
本项目是从 `xincmm/sageread` fork 而来,[xincmm/sageread](https://github.com/xincmm/sageread) 长时间未更新，我们在其基础上持续迭代优化，欢迎更多小伙伴加入共建。

该分支目前由我持续维护，
包含新的功能、问题修复以及各类改进。
本项目基于 GNU AGPL v3.0 协议发布。

## 📒更新日志
| 日期 | 新功能 | Status |
| :--- | :--- | :---: |
| 2025.12.6| 增加 pdf 导入&划词翻译功能| ✅ | 
| 2025.12.9| 增加笔记编辑功能| ✅ | 
| 2025.12.14| 增加分页模式下左右滑动翻页的功能| ✅ | 
| 2025.12.14| 增加笔记&标注导出功能| ✅ | 
| 2025.12.14| 优化总结本章功能,调整为获取当前页内容进行总结| ✅ | 
| 2025.12.14| **修改项目名称为SageRead X**，数据和原项目隔离!| ✅ | 
| 2025.12.17| 修复部分场景epub 导入失败的问题| ✅ | 
| 2025.12.22| 修复向量化失败的问题| ✅ | 
| 2025.12.22| 支持英语| ✅ | 
| 2025.12.24| 优化翻译交互| ✅ |
| 2026.1.1| 支持txt文本的导入| ✅ |
| 2026.1.1| 支持韩语、日语等语言| ✅ |
| 2026.1.2| linux版本支持| ⚙️|
| 2026.1.4| 模型选择区分翻译和对话| ✅ |
| 2026.1.10| 修复若干问题&增加公共标注提示功能| ✅ |
| 2026.2.1| 引入「简约模式」入口，提供更轻量的阅读+AI对话体验，**注意：由于人力有限，v2.0.0+版本只支持这种模式**| ✅ |
| 2026.3.8| 修复 session 失效未重新登录的问题| ✅ |
| 2026.3.11| 分页模式支持鼠标滚轮上下翻页（含阈值优化）| ✅ |

## 🚀进一步交流`
加入群聊,期待获取更多的意见和建议📊 
**WeChat**  
- 2026.3.15日前有效
<img src="assets/community/qunliao.JPG" alt="community" width="140" />
